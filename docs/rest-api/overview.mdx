...
The REST API imposes a limit of 200 requests per second by project for writing operations (`POST`, `PUT` and `DELETE`). Reading operations have a limit of 400 requests per second. It responds with the 429 HTTP status if the limit is exceeded.
