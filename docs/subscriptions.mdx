...
The backend system creates the subscription via the API. When the user clicks the subscribe button a topic subscription record is created with the status of subscribed. Whenever the topic is used in a notification the user will receive the notification.
