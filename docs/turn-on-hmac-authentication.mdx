...
class Main {
  public static void main(String[] args) throws InvalidKeyException, NoSuchAlgorithmException {
    String hmac = MagicBellAuth.calculateHMAC("<external_id>", "MAGICBELL_API_SECRET");
    System.out.println(hmac);
  }
}
```
...
const userExternalIDHMAC = Base64.stringify(hmacSHA256(userExternalID, MAGICBELL_API_SECRET));
```
...
export default function Notifications() {
  return (
    <MagicBell apiKey="MAGICBELL_API_KEY" userExternalID="<external_id>" userKey="userExternalIDHmac">
      {(props) => <FloatingNotificationInbox height={500} {...props} />}
    </MagicBell>
  );
)
